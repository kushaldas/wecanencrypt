...
fn wecanencrypt_encrypt_sqop_decrypt() {
    let Some(sqop) = skip_if_no_sqop() else { return };
...
fn sqop_sign_wecanencrypt_verify() {
    ...
    let valid = verify_bytes_detached(pub_key.as_bytes(), PLAINTEXT, &sig)
        .expect("verify_bytes_detached");
    assert!(valid, "wecanencrypt should accept sqop's detached signature");
}
