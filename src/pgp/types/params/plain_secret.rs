    ...
    fn try_from_reader_inner<B: BufRead>(
        ...
        let params = match (alg, public_params) {
            (
                ...
                let key = crate::pgp::crypto::rsa::SecretKey::try_from_mpi(pub_params, d, p, q, u)?;
                Self::RSA(key)
            }
            (PublicKeyAlgorithm::DSA, PublicParams::DSA(pub_params)) => {
                let secret = Mpi::try_from_reader(i)?;

                let key = crate::pgp::crypto::dsa::SecretKey::try_from_mpi(pub_params, secret)?;
                Self::DSA(key)
            }
            (PublicKeyAlgorithm::Elgamal, PublicParams::Elgamal(pub_params)) => {
                ...
                let key = crate::pgp::crypto::elgamal::SecretKey::try_from_mpi(pub_params.clone(), x);
                Self::Elgamal(key)
            }
            ...
            (PublicKeyAlgorithm::ECDH, PublicParams::ECDH(pub_params)) => {
                ...
                let key = crate::pgp::crypto::ecdh::SecretKey::try_from_mpi(pub_params, secret)?;
                Self::ECDH(key)
            }
            (PublicKeyAlgorithm::ECDSA, PublicParams::ECDSA(pub_params)) => {
                let secret = Mpi::try_from_reader(i)?;

                let key = crate::pgp::crypto::ecdsa::SecretKey::try_from_mpi(pub_params, secret)?;
                Self::ECDSA(key)
            }
            (PublicKeyAlgorithm::EdDSALegacy, PublicParams::EdDSALegacy(_pub_params)) => {
                ...
                let key = crate::pgp::crypto::ed25519::SecretKey::try_from_bytes(
                    secret,
                    crate::pgp::crypto::ed25519::Mode::EdDSALegacy,
                )?;
            ...
            (PublicKeyAlgorithm::Ed25519, PublicParams::Ed25519(_pub_params)) => {
                ...
                let key = crate::pgp::crypto::ed25519::SecretKey::try_from_bytes(
                    secret,
                    crate::pgp::crypto::ed25519::Mode::Ed25519,
                )?;
                Self::Ed25519(key)
            }
            (PublicKeyAlgorithm::Ed448, PublicParams::Ed448(_pub_params)) => {
                let secret = i.read_array::<57>()?;
                let key = crate::pgp::crypto::ed448::SecretKey::try_from_bytes(secret)?;
                Self::Ed448(key)
            }
            (PublicKeyAlgorithm::X25519, PublicParams::X25519(_)) => {
                let secret = i.read_array::<32>()?;
                let key = crate::pgp::crypto::x25519::SecretKey::try_from_bytes(secret)?;
                Self::X25519(key)
            }
            (PublicKeyAlgorithm::X448, PublicParams::X448 { .. }) => {
                let s = i.read_array::<56>()?;
                let key = crate::pgp::crypto::x448::SecretKey::try_from_bytes(s)?;
                Self::X448(key)
            }
            #[cfg(feature = "draft-pqc")]
            (PublicKeyAlgorithm::MlKem768X25519, PublicParams::MlKem768X25519(_)) => {
                ...
                let key = crate::pgp::crypto::ml_kem768_x25519::SecretKey::try_from_bytes(x, ml_kem)?;
            ...
            (PublicKeyAlgorithm::MlKem1024X448, PublicParams::MlKem1024X448(_)) => {
                ...
                // ML KEM
                let ml_kem = i.read_array::<64>()?;
                let key = crate::pgp::crypto::ml_kem1024_x448::SecretKey::try_from_bytes(x, ml_kem)?;
            ...
            (PublicKeyAlgorithm::MlDsa65Ed25519, PublicParams::MlDsa65Ed25519(_)) => {
                ...
                let key = crate::pgp::crypto::ml_dsa65_ed25519::SecretKey::try_from_bytes(ed, ml_dsa)?;
        ...
        let decrypted_key = match (self, values) {
            ...
            (
                ...
                if let EcdhPublicParams::Curve25519 {
                    ecdh_kdf_type:
                        crate::pgp::types::EcdhKdfType::Replaced {
            ...
            (
                ...
                let data = crate::pgp::crypto::x448::EncryptionFields {
                    ephemeral_public_point: ephemeral.to_owned(),
