...
pub use crate::pgp::parsing::{Error as ParsingError, RemainingError};
...
pub enum Error {
    ...
    Aead { source: crate::pgp::crypto::aead::Error },
    #[snafu(display("AES key wrap {:?}", source), context(false))]
    AesKw {
        source: crate::pgp::crypto::aes_kw::Error,
    },
    #[snafu(transparent)]
    ChecksumMissmatch {
        source: crate::pgp::crypto::checksum::ChecksumMismatch,
    },
    #[snafu(transparent)]
    Sha1HashCollision {
        source: crate::pgp::crypto::checksum::Sha1HashCollision,
    },
    #[snafu(transparent)]
...
impl From<crate::pgp::crypto::hash::Error> for Error {
    fn from(err: crate::pgp::crypto::hash::Error) -> Self {
        match err {
            crate::pgp::crypto::hash::Error::Unsupported { alg } => UnsupportedSnafu {
                message: format!("hash algorithm: {alg:?}"),
            }
            .build(),
            crate::pgp::crypto::hash::Error::Sha1HashCollision { source } => source.into(),
        }
    }
}
...
macro_rules! unimplemented_err {
    ($e:expr) => {
        return Err($crate::pgp::errors::UnimplementedSnafu { message: $e.to_string() }.build())
    };
    ($fmt:expr, $($arg:tt)+) => {
        return Err($crate::pgp::errors::UnimplementedSnafu { message: format!($fmt, $($arg)+)}.build())
    };
}

macro_rules! unsupported_err {
    ($e:expr) => {
        return Err($crate::pgp::errors::UnsupportedSnafu {
            message: $e.to_string(),
        }.build())
    };
    ($fmt:expr, $($arg:tt)+) => {
...
macro_rules! bail {
    ($e:expr) => {
        return Err($crate::pgp::errors::Error::Message {
            message: $e.to_string(),
...
macro_rules! format_err {
    ($e:expr) => {
        $crate::pgp::errors::Error::Message {
            message: $e.to_string(),
            backtrace: ::snafu::GenerateImplicitData::generate(),
        }
    };
...
macro_rules! ensure {
    ($cond:expr, $e:expr) => {
        if !($cond) {
            $crate::pgp::errors::bail!($e);
        }
    };
    ($cond:expr, $fmt:expr, $($arg:tt)+) => {
        if !($cond) {
            $crate::pgp::errors::bail!($fmt, $($arg)+);
        }
    };
}
...
macro_rules! ensure_eq {
        ...
        match (&$left, &$right) {
            (left_val, right_val) => {
                if !(*left_val == *right_val) {
                    $crate::pgp::errors::bail!(r#"assertion failed: `(left == right)`
  left: `{:?}`,
 right: `{:?}`"#, left_val, right_val)
    ...
    ($left:expr, $right:expr,) => ({
        $crate::pgp::errors::ensure_eq!($left, $right)
    });
