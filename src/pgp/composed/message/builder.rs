...
) -> Result<Vec<(crate::pgp::packet::SignatureConfig, OnePassSignature)>>
where
    R: Rng + CryptoRng,
{
    let mut out = Vec::new();
    ...
    for (i, config) in keys.iter().enumerate() {
        ...
        let mut sig_config = match config.key.version() {
            KeyVersion::V4 => crate::pgp::packet::SignatureConfig::v4(typ, algorithm, hash_alg),
            KeyVersion::V6 => {
                crate::pgp::packet::SignatureConfig::v6(&mut rng, typ, algorithm, hash_alg)?
            }
...
mod tests {
    ...
    fn binary_file_fixed_size_no_compression_roundtrip_password_seipdv1() {
        ...
        for file_size in (1..=max_file_size).step_by(10) {
            ...
            // encrypt it
            let s2k = crate::pgp::types::StringToKey::new_iterated(&mut rng, Default::default(), 2);
