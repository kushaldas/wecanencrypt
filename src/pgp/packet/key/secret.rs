...
impl SecretKey {
    pub fn new(details: super::PublicKey, secret_params: SecretParams) -> Result<Self> {
        let len =
            crate::pgp::ser::Serialize::write_len(&details) + secret_params.write_len(details.version());
    ...
    pub fn try_from_reader<B: BufRead>(packet_header: PacketHeader, input: B) -> Result<Self> {
        ...
        let details = crate::pgp::packet::secret_key_parser::parse(input)?;
        ...
        let pub_packet_header = PacketHeader::from_parts(
            ...
            Tag::PublicKey,
            crate::pgp::types::PacketLength::Fixed(len.try_into()?),
        )?;
...
impl crate::pgp::ser::Serialize for SecretKey {
    fn to_writer<W: std::io::Write>(&self, writer: &mut W) -> Result<()> {
        // writes version and public part
        crate::pgp::ser::Serialize::to_writer(&self.details, writer)?;
        self.secret_params.to_writer(writer, self.version())?;
        Ok(())
    }

    fn write_len(&self) -> usize {
        let details_len = crate::pgp::ser::Serialize::write_len(&self.details);
...
impl crate::pgp::ser::Serialize for SecretSubkey {
    fn to_writer<W: std::io::Write>(&self, writer: &mut W) -> Result<()> {
        // writes version and public part
        crate::pgp::ser::Serialize::to_writer(&self.details, writer)?;
        self.secret_params.to_writer(writer, self.version())?;
...
impl SecretKey {
    ...
    {
        let s2k = crate::pgp::types::S2kParams::new_default(rng, self.version());
    ...
    pub fn set_password_with_s2k(
        &mut self,
        password: &Password,
        s2k_params: crate::pgp::types::S2kParams,
    ) -> Result<()> {
...
mod tests {
    ...
    fn secret_key_protection_v4() {
        ...
        let key_type = crate::pgp::composed::KeyType::Ed25519Legacy;
        ...
        .unwrap();
        let pub_key = crate::pgp::packet::PublicKey::from_inner(pub_key).unwrap();
        let mut alice_sec = SecretKey::new(pub_key, secret_params).unwrap();

        alice_sec
            .set_password_with_s2k(
                &"password".into(),
                crate::pgp::types::S2kParams::new_default(&mut rng, KeyVersion::V4),
            )
            .unwrap();
